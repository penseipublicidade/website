...
* `begin` - An instance of [`Alignment`](/docs/reference/types/alignment) class. The offset at which stop 0.0 of the gradient is placed.
* `end` - An instance of [`Alignment`](/docs/reference/types/alignment) class. The offset at which stop 1.0 of the gradient is placed.
* `colors` - The [`colors`](/docs/reference/colors) the gradient should obtain at each of the stops. If stops is non-null, this list must have the same length as stops. This list must have at least two colors in it (otherwise, it's not a gradient!).
